...
#include "ss_cn_font.h"             // Chinese bitmap font for the bottom-screen labels
...
static const char *const kDungeonNames[14] = {
  "下水道", "海拉鲁城堡", "东方神殿", "沙漠神殿", "城堡塔楼",
  "水之祠堂", "暗之神殿", "恶魔之沼", "头骨之森", "冰之塔",
  "赫拉之塔", "盗贼之村", "乌龟岩", "加侬之塔",
};
...
static SDL_Texture *tex_map[2], *tex_icons, *tex_glyphs, *tex_letters, *tex_face, *tex_cn;
...
// Chinese text drawn from the embedded bitmap font (ss_cn_font.h).
static int cn_cell_of(uint32_t cp) {
  for (int i = 0; i < SS_CN_COUNT; i++)
    if (kSS_CnCell[i] == cp) return i;
  return -1;
}
static float cn_text_width(const char *s, float sc) {
  float w = 0;
  for (const unsigned char *p = (const unsigned char *)s; *p; ) {
    ...
    if (cn_cell_of(cp) >= 0) w += SS_CN_PX * sc;
    else if (cp == ' ') w += 5 * sc;
    else if (cp < 0x80) w += text_width((const char *)&cp, sc);  // ASCII fallback
  }
  return w;
}
static void draw_cn_text_col(const char *s, float x, float y, float sc, uint32_t color) {
  y -= 3.0f;  // nudge Chinese labels a few pixels up for better vertical fit
  uint8_t cr = (color >> 16) & 0xff, cg = (color >> 8) & 0xff, cb = color & 0xff;
  SDL_SetTextureColorMod(tex_cn, cr, cg, cb);
  SDL_SetTextureColorMod(tex_glyphs, cr, cg, cb);
  SDL_SetTextureColorMod(tex_letters, cr, cg, cb);
  float cx = x;
  for (const unsigned char *p = (const unsigned char *)s; *p; ) {
    uint32_t cp;
    if (*p < 0x80) { cp = *p; p += 1; }
    ...
    if (cp == ' ') { cx += 5 * sc; continue; }
    int cell = cn_cell_of(cp);
    if (cell >= 0) {
      draw_cell(tex_cn, cell, SS_CN_PX, SS_CN_COLS, cx, y, sc);
      cx += SS_CN_PX * sc;
    } else if (cp < 0x80) {
      char tmp[2] = { (char)cp, 0 };  // ASCII fallback (digits, HUD letters, X...)
      draw_text(tmp, cx, y, sc);
      cx += text_width(tmp, sc);
    }
  }
  SDL_SetTextureColorMod(tex_cn, 255, 255, 255);
  SDL_SetTextureColorMod(tex_glyphs, 255, 255, 255);
  SDL_SetTextureColorMod(tex_letters, 255, 255, 255);
}

static void draw_cn_text(const char *s, float x, float y, float sc) {
  draw_cn_text_col(s, x, y, sc, COL(255, 255, 255));
}
...
static bool try_load_art(void) {
  ...
  tex_cn = make_tex(SS_CN_COLS * SS_CN_PX, SS_CN_ROWS * SS_CN_PX, (uint32_t *)kSS_CnGlyph, true);
...
static void draw_dungeon(RectFS r, int link_x, int link_y, int room, int dungeon_info) {
  ...
  float bs = 3 * u;
  const char *name = d ? d->name : "地下城";
  float tw = cn_text_width(name, bs);
...
static void draw_items(RectFS r) {
  ...
  draw_cn_text("物品", r.x + r.w / 2 - cn_text_width("物品", 3 * u) / 2, r.y + 18 * u, 3 * u);
...
static void draw_gear(RectFS r) {
  ...
  draw_cn_text("装备", r.x + r.w / 2 - cn_text_width("装备", 2.0f) / 2,
               r.y + 12.0f, 2.0f);
  ...
  draw_cn_text("瓶子", x0, y1 - 12.0f, 1.0f);
  for (int i = 0; i < 4; i++) {
  ...
  draw_cn_text("吊坠", pend_x, y1 - 12.0f, 1.0f);
  int pend = sram8(0x74);
  ...
  float cyC = y1 + 49.0f;
  draw_cn_text("水晶", x0, cyC - 12.0f, 1.0f);
...
static void draw_remap_panel(RectFS r) {
  draw_cn_text("重映射按键", r.x + r.w / 2 - cn_text_width("重映射按键", 3 * u) / 2,
            r.y + 18 * u, 3 * u);
  remap_back_r = (RectFS){r.x + 14 * u, r.y + 12 * u, 76 * u, 32 * u};
  draw_settings_row(&remap_back_r, false);
  draw_cn_text("返回", remap_back_r.x + remap_back_r.w / 2 - cn_text_width("返回", 1.8f * u) / 2,
  ...
  draw_cn_text(remap_first_row == 0 ? "更多" : "顶部",
            remap_page_r.x + remap_page_r.w / 2 -
              cn_text_width(remap_first_row == 0 ? "更多" : "顶部", 1.8f * u) / 2,
  ...
  for (int visible = 0; visible < 6; visible++) {
    ...
    *row = (RectFS){r.x + 14 * u, y0 + visible * (row_h + gap), row_w, row_h};
    ...
    draw_cn_text(kPadCmdNames[i], row->x + 12 * u, ty, 2.1f * u);
...
static const char *display_mode_label(void) {
  ...
  case kPlatform3DSDisplayOriginal: return "原始";
  case kPlatform3DSDisplayStretch: return "拉伸";
  case kPlatform3DSDisplayUltraWideMod:
  default: return "宽屏";
  }
#else
  return SS_IsWidescreen() ? "宽屏" : "原始";
#endif
}
...
static void draw_screen_panel(RectFS r) {
  draw_cn_text("屏幕", r.x + r.w / 2 - cn_text_width("屏幕", 3 * u) / 2,
            r.y + 18 * u, 3 * u);
  screen_back_r = (RectFS){r.x + 20 * u, r.y + 12 * u, 90 * u, 38 * u};
  draw_settings_row(&screen_back_r, false);
  draw_cn_text("返回", screen_back_r.x + screen_back_r.w / 2 - cn_text_width("返回", 2.2f * u) / 2,
            screen_back_r.y + screen_back_r.h / 2 - 9 * u, 2.2f * u);

  const char *edge_value = "固定镜头";
#ifdef __3DS__
  ...
  for (int visible = 0; visible < rows; visible++) {
    ...
    *row = (RectFS){
    ...
    draw_cn_text(labels[item], row->x + 16 * u, ty, 2 * u);
    draw_cn_text(values[item],
              row->x + row->w - 16 * u - cn_text_width(values[item], 2 * u),
              ty, 2 * u);
  }
}

static void draw_developer_panel(RectFS r) {
  draw_cn_text("开发者", r.x + r.w / 2 - cn_text_width("开发者", 3 * u) / 2,
            r.y + 18 * u, 3 * u);
  developer_back_r = (RectFS){r.x + 20 * u, r.y + 12 * u, 90 * u, 38 * u};
  draw_settings_row(&developer_back_r, false);
  draw_cn_text("返回", developer_back_r.x + developer_back_r.w / 2 - cn_text_width("返回", 2.2f * u) / 2,
            developer_back_r.y + developer_back_r.h / 2 - 9 * u, 2.2f * u);

  const char *labels[2] = {"内存转储", "叠加"};
  const char *values[2] = {
    SDL_GetTicks() < dump_flash_until ? "完成" : "写入",
    "打开",
  };
  float row_h = 58 * u, gap = 14 * u;
  float y0 = r.y + 82 * u;
  for (int i = 0; i < 2; i++) {
    ...
    *row = (RectFS){r.x + 28 * u, y0 + i * (row_h + gap), r.w - 56 * u, row_h};
    draw_settings_row(row, false);
    float ty = row->y + row->h / 2 - 8 * u;
    draw_cn_text(labels[i], row->x + 16 * u, ty, 2 * u);
    draw_cn_text(values[i], row->x + row->w - 16 * u - cn_text_width(values[i], 2 * u),
              ty, 2 * u);
  }
}

static void draw_developer_overlay_panel(RectFS r) {
  draw_cn_text("叠加", r.x + r.w / 2 - cn_text_width("叠加", 3 * u) / 2,
            r.y + 18 * u, 3 * u);
  developer_back_r = (RectFS){r.x + 20 * u, r.y + 12 * u, 90 * u, 38 * u};
  draw_settings_row(&developer_back_r, false);
  draw_cn_text("返回", developer_back_r.x + developer_back_r.w / 2 - cn_text_width("返回", 2.2f * u) / 2,
  ...
  snprintf(model, sizeof(model), "%s",
           Platform3DS_IsNew3DS() ? "新 3DS" : "旧 3DS");
  snprintf(core, sizeof(core), "%s",
           Platform3DS_CanUseCore1PpuWorker() ? "开" : "关");
#else
  snprintf(model, sizeof(model), "桌面");
  snprintf(core, sizeof(core), "无");
#endif
  ...
  if (SS_IsIndoors()) {
    ...
    if (palace >= 0 && palace < 14)
      ...
      snprintf(location, sizeof(location), "房间 %02X", SS_GetArea() & 0xff);
  } else {
    snprintf(location, sizeof(location), "野外 %02X", SS_GetArea() & 0xff);
  }
  ...
  struct {
    ...
    {"版本", version, COL(255, 255, 255)},
    {"机型", model, COL_GOLD},
    {"当前帧率", fps_now, COL(120, 255, 140)},
    {"平均帧率", fps_avg, COL(120, 220, 255)},
    {"核心1", core, COL(230, 230, 230)},
    {"屏幕", display, COL(230, 230, 230)},
    {"房间", location, COL(230, 230, 230)},
    {"模块", module, COL(230, 230, 230)},
  };
  for (size_t i = 0; i < sizeof(rows) / sizeof(rows[0]); i++) {
    fill_round(x - 8 * u, y - 4 * u, r.w - 40 * u, row_h, 5 * u,
               i == 2 || i == 3 ? COL(34, 28, 12) : COL(24, 24, 24));
    draw_cn_text_col(rows[i].label, x, y, sc, COL(170, 170, 170));
    draw_cn_text_col(rows[i].value, x + 180 * u, y, sc, rows[i].color);
    y += row_h + 3 * u;
  }
}
...
static void draw_settings(RectFS r) {
  ...
  draw_cn_text("设置", r.x + r.w / 2 - cn_text_width("设置", 3 * u) / 2, r.y + 18 * u, 3 * u);
  ...
  for (int i = 0; i < 6; i++) {
    ...
    *row = (RectFS){r.x + 28 * u, y0 + i * (row_h + gap), r.w - 56 * u, row_h};
    ...
    if (values[i][0] == 0) {
      ...
      draw_cn_text(values[i], row->x + row->w - 16 * u - cn_text_width(values[i], 2 * u), ty, 2 * u);
    }
  }
}
...
static void draw_tab_button(RectFS r, const char *label, bool active) {
  ...
  float s = 3 * u;
  if (label)
    draw_cn_text(label, r.x + r.w / 2 - cn_text_width(label, s) / 2, r.y + r.h / 2 - 4 * s, s);
}
...
static void draw_tab_bar(float tab_h) {
  ...
  draw_tab_button(tab_gear_r, "装备", tab == TAB_GEAR);
  draw_tab_button(tab_map_r, "地图", tab == TAB_MAP);
  draw_tab_button(tab_items_r, "物品", tab == TAB_ITEMS);
